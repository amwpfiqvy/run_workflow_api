...
DEFAULT_PROXY = ""
POLL_SECONDS = 20
LOG_DIR = Path(os.environ.get("WATCHDOG_LOG_DIR", "/var/log/run-workflow-api"))
...
def run(args: argparse.Namespace) -> str:
    ...
    if age < args.threshold:
        return f"检查完成：最近执行距现在 {int(age)} 秒，不足 {args.threshold} 秒，未执行"

    if args.dry_run:
        return f"只读检查：距最近运行 {int(age)} 秒，达到阈值，本应触发但未执行"
    ...
    raise WatchdogError("dispatch 已提交，但未确认新运行记录；不自动重发，请人工检查")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--dry-run", action="store_true", help="只读检查，不提交 dispatch")
    parser.add_argument(
        "--threshold",
        type=int,
        default=os.environ.get("WATCHDOG_THRESHOLD", str(THRESHOLD_SECONDS)),
        help="距最近一次运行的最小秒数，默认420",
    )
    parser.add_argument(
        "--ref",
        default=os.environ.get("WATCHDOG_REF", DEFAULT_REF),
    ...
    parser.add_argument(
        ...
        or os.environ.get("http_proxy")
        or DEFAULT_PROXY,
        help="默认使用代理环境变量，无配置则直连；传空字符串强制直连",
    )
    args = parser.parse_args()
    if args.threshold < 0:
        parser.error("--threshold 不能为负数")
    if args.proxy == "":
        args.proxy = None
    return args
...
def main() -> None:
    ...
    result = f"{OWNER}/{REPO}/{WORKFLOW_FILE} {result}"
    write_log(result)
